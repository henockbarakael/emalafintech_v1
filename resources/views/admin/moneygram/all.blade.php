            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="card-header">
                        ...
                        <p class="card-text">
                            Le tableau ci-dessous affiche l'historique de toutes les transactions <code>Moneygram</code>. Il est possible d'exporter le fchier au format désiré.
                        </p>
                    </div>
                    <div class="card-body">
                            ...
                            <div class="table-responsive">
                                <table id="example1" class="table table-bordered table-stripped mb-0">
                                    ...
                                    <tbody>
                                        @foreach ($moneygram as $key=>$moneygram )
                                        <tr>
                                            <td>{{ ++$key }}</td>
                                            <td>{{$moneygram->sender_fullname}}</td>
                                            <td  class="montant">{{ $moneygram->amount." ".$moneygram->currency }}</td>
                                            {{-- <td  class="e_money">{{ $moneygram->currency }}</td> --}}
                                            <td  class="e_desitinateur">{{ $moneygram->receiver_fullname }}</td>
                                            <td class="e_current">{{ $moneygram->transaction_id }}</td>
                                            <td class="e_current">{{ $moneygram->transaction_type }}</td>
                                            <td class="e_current">{{ $moneygram->created_at }}</td>
                                            <td class="text-center">
												<div class="dropdown action-label">
                                                    @if ($moneygram->status == "Pending")
                                                    <a class="btn btn-warning btn-sm " href="#"  aria-expanded="false">
														<i class="fa fa-dot-circle-o text-white"></i> {{ $moneygram->status }}
													</a>
                                                    @elseif ($moneygram->status == "Failed")
