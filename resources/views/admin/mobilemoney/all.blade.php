            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="card-header">
                        ...
                        <p class="card-text">
                            Le tableau ci-dessous affiche l'historique de toutes les transactions <code>Mobile money</code>. Il est possible d'exporter le fchier au format désiré.
                        </p>
                    </div>
                    <div class="card-body">
                            ...
                            <div class="table-responsive">
                                <table id="example1" class="table table-bordered table-stripped mb-0">
                                    ...
                                    <tbody>
                                        @foreach ($mobilemoney as $key=>$mobilemoney )
                                        <tr>
                                            <td>{{ ++$key }}</td>
                                            <td>{{$mobilemoney->sender_fullname}}</td>
                                            <td  class="montant">{{ $mobilemoney->amount." ".$mobilemoney->currency }}</td>
                                            {{-- <td  class="e_money">{{ $mobilemoney->currency }}</td> --}}
                                            <td  class="e_desitinateur">{{ $mobilemoney->receiver_fullname }}</td>
                                            <td class="e_current">{{ $mobilemoney->transaction_id }}</td>
                                            <td class="e_current">{{ $mobilemoney->transaction_type }}</td>
                                            <td class="e_current">{{ $mobilemoney->created_at }}</td>
                                            <td class="text-center">
												<div class="dropdown action-label">
                                                    @if ($mobilemoney->status == "Pending")
                                                    <a class="btn btn-warning btn-sm " href="#"  aria-expanded="false">
														<i class="fa fa-dot-circle-o text-white"></i> {{ $mobilemoney->status }}
													</a>
                                                    @elseif ($mobilemoney->status == "Failed")
